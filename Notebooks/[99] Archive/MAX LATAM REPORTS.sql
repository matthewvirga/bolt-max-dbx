...
-- DBTITLE 1,Plan switch for installment plans
with prev_sub_type as 
(
  select s.*, p.*
  from subscriptions s
  join plans p on p.priceplanid = s.priceplanid
)
...
s.baseSubscriptionId,
s.previousSubscriptionGlobalId,
replace(ps.priceplantype, 'TYPE_','') as prev_priceplan_type,
s.userid

from Subscriptions s
left join plans p on s.priceplanid = p.priceplanid
left join prev_sub_type ps on s.previousSubscriptionGlobalId = ps.globalsubscriptionid
-- left join latest_charge lc on s.globalsubscriptionid = lc.source_reference
-- left join prev_subs
-- left join subscriptions s2 on s.globalsubscriptionid = s2.previousSubscriptionGlobalId

where 1=1
and s.previousSubscriptionGlobalId is not null
and p.priceplantype = 'TYPE_INSTALMENTS'
group by all
having provider_type in ('Direct')
-- and prev_priceplan_type = 'INSTALMENTS'
-- )

-- COMMAND ----------

-- DBTITLE 1,TNT Standalone Subs and Payments
select * 
...
(
  ...
  from plans
  where region = 'LATAM'
  and upper(productname) LIKE '%TNT%'
);
